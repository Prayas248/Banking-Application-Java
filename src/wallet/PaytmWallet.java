...
// Paytm wallet with a ₹50,000 maximum balance limit
public class PaytmWallet implements WalletOperations, PaymentType {
    FileLogger logger; // used to log errors to a file when wallet limit is exceeded
    Customer customer;
    double walletBalance;

    public PaytmWallet(Customer customer) {
        this.customer = customer;
        System.out.println("Paytm Wallet created successfully for " + customer.getCustomerName());
    }
    ...
    public void addMoney(double amount) throws WalletLimitExceededException, InvalidAmountException {
        ...
        // Log to file and throw if the wallet would exceed ₹50,000
        ...
        this.customer.addTransaction(amount, "Paytm", this.walletBalance);
        System.out.println("Added ₹" + amount + " to Paytm wallet | New Balance: ₹" + this.walletBalance);
    }
    ...
    public void payBill(double amount) throws InsufficientBalanceException, InvalidAmountException {
        ...
        this.walletBalance = updatedBalance;
        this.customer.addTransaction(amount, "Paytm", this.walletBalance);
        System.out.println("Bill of ₹" + amount + " paid via Paytm | Remaining Balance: ₹" + this.walletBalance);
    }
    ...
    public void transferToWallet(Customer receiver, double amount) throws Exception {
        ...
        // Receiver must also have a Paytm wallet — cross-wallet transfers are not supported
        PaymentType receiverWallet = receiver.getAccount("Paytm");
        if (!(receiverWallet instanceof PaytmWallet)) {
            throw new AccountNotFound("Receiver does not have a Paytm wallet.");
        }
        this.walletBalance = updatedBalance;
        this.customer.addTransaction(amount, "Paytm", this.walletBalance);
        ((PaytmWallet) receiverWallet).addMoney(amount);
        System.out.println("Transferred ₹" + amount + " to " + receiver.getCustomerName() + "'s Paytm wallet | Your Balance: ₹" + this.walletBalance);
    }
}
